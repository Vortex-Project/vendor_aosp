...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop org.vortex.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop org.vortex.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.vortex.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
